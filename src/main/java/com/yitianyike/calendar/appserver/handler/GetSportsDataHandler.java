...
public class GetSportsDataHandler {
	private static Logger logger = Logger.getLogger(IncrementDataHandler.class);
	private GetSportsDataDB getSportsDataDB = (GetSportsDataDB) DataAccessFactory.dataHolder().get("getSportsDataDB");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public GetSportsDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_GREEN + "getsportsdata handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = getSportsDataDB.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("getsportsdata handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") "
				+ displayStr);
