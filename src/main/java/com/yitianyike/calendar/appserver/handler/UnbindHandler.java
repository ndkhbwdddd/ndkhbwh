...
public class UnbindHandler {
	private static Logger logger = Logger.getLogger(UnbindHandler.class);
	private UnbindBO unbindBO = (UnbindBO) DataAccessFactory.dataHolder().get("unbindBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public UnbindHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_GREEN + "unbind handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = unbindBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("unbind handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
