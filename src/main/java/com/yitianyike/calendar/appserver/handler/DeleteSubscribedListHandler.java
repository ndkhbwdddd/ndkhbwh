 ...
 *
 */
public class DeleteSubscribedListHandler {
	private static Logger logger = Logger.getLogger(DeleteSubscribedListHandler.class);
	private DeleteSubscribedListBO deleteSubscribedListBO = (DeleteSubscribedListBO) DataAccessFactory.dataHolder()
			.get("deleteSubscribedListBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public DeleteSubscribedListHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_YELLOW + "deleteSubscribedlist handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = deleteSubscribedListBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("deleteSubscribedlist handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") "
				+ displayStr);
