...
public class UpdateDataHandler {
    private static Logger logger = Logger.getLogger(UpdateDataHandler.class);
    private UpdateDataBO updateDataBO = (UpdateDataBO) DataAccessFactory.dataHolder().get("updateDataBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public UpdateDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "updatedata handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = updateDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("updatedata handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
	...
	public void updateByDate() {
    	...
    	AppResponse resp = updateDataBO.updateByDate(parmMap, content, requestIndex);
