...
public class LoginHandler {
	...
	private LoginBO loginBO = (LoginBO) DataAccessFactory.dataHolder().get("loginBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public LoginHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_CYAN + "login handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = loginBO.process(parmMap, content, requestIndex);
		...
		logger.info("login handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
