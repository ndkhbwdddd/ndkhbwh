 ...
 *
 */
public class MergeSubscriptionsHandler {
	private static Logger logger = Logger.getLogger(MergeSubscriptionsHandler.class);
	private MergeSubscriptionsBO mergeSubscriptionsBO = (MergeSubscriptionsBO) DataAccessFactory.dataHolder()
			.get("mergeSubscriptionsBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public MergeSubscriptionsHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_GREEN + "mergeSubscriptions handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = mergeSubscriptionsBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("mergeSubscriptions handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") "
				+ displayStr);
