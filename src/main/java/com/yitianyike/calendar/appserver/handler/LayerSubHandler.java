...
public class LayerSubHandler {
    private static Logger logger = Logger.getLogger(LayerSubHandler.class);
    private LayerSubDataBO layerSubDataBO = (LayerSubDataBO) DataAccessFactory.dataHolder().get("layerSubDataBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public LayerSubHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "LayerSubHandler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = layerSubDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("LayerSubHandler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
