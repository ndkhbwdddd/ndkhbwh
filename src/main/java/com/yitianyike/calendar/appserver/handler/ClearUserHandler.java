...
public class ClearUserHandler {
	private static Logger logger = Logger.getLogger(ClearUserHandler.class);
	private ClearUserBO clearUserBO = (ClearUserBO) DataAccessFactory.dataHolder().get("clearUserBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public ClearUserHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_GREEN + "ClearUserHandler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = clearUserBO.process(parmMap, content, requestIndex);
		...
		logger.info(
				"ClearUserHandler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
