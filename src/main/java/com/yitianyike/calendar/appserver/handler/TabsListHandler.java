 ...
 *
 */
public class TabsListHandler {
    private static Logger logger = Logger.getLogger(TabsListHandler.class);
    ...
    public TabsListHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_YELLOW + "tabsListHandler handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = tabsListBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("tabsListHandler handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
