 ...
 *
 */
public class SynchronizeDataHandler {
    private static Logger logger = Logger.getLogger(SynchronizeDataHandler.class);
    private SynchronizeDataBO synchronizeDataBO = (SynchronizeDataBO) DataAccessFactory.dataHolder().get("synchronizeDataBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public SynchronizeDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "synchronizedata handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = synchronizeDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("synchronizedata handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
