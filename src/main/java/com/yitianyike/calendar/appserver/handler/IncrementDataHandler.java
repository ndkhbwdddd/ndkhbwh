 ...
 *
 */
public class IncrementDataHandler {
    private static Logger logger = Logger.getLogger(IncrementDataHandler.class);
    private IncrementDataBO incrementDataBO = (IncrementDataBO) DataAccessFactory.dataHolder().get("incrementDataBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public IncrementDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "incrementdata handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = incrementDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("incrementdata handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
	 ...
	 void getSportsData() {
    	...
    	AppResponse resp = incrementDataBO.getSportsData(parmMap, content, requestIndex);
