 ...
 *
 */
public class UpdatePeriodDataHandler {
	private static Logger logger = Logger.getLogger(UpdateDataHandler.class);
	private UpdatePeriodDataBO updatePeriodDataBO = (UpdatePeriodDataBO) DataAccessFactory.dataHolder()
			.get("updatePeriodDataBO");

	private Map<String, String> parmMap;
	private ChannelHandlerContext ctx;
	private String content;

	public UpdatePeriodDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
		this.parmMap = map;
		this.ctx = ctx;
	...
	void process() {
		...
		logger.info(EnumConstants.COLOR_GREEN + "updateperiod handler param(" + requestIndex + ") : " + parmMap
				+ EnumConstants.COLOR_NONE);

		AppResponse resp = updatePeriodDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info(
				"updateperiod handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
