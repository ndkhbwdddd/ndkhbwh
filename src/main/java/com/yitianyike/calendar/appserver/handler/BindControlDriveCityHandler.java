 ...
 *
 */
public class BindControlDriveCityHandler {
    private static Logger logger = Logger.getLogger(BindControlDriveCityHandler.class);
    private BindControlDriveCityBO bindControlDriveCityBO = (BindControlDriveCityBO) DataAccessFactory.dataHolder().get("bindControlDriveCityBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public BindControlDriveCityHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "BindControlDriveCityHandler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = bindControlDriveCityBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("BindControlDriveCityHandler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
