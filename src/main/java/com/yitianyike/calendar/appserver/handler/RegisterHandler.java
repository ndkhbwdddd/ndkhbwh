...
public class RegisterHandler {
    ...
    private RegisterBO registerBO = (RegisterBO) DataAccessFactory.dataHolder().get("registerBO");
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_CYAN + "register handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = registerBO.process(parmMap, content, requestIndex);
		...
		logger.info("register handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
