 ...
 *
 */
public class SubscribedListHandler {
    private static Logger logger = Logger.getLogger(SubscribedListHandler.class);
    private SubscribedListBO subscribedListBO = (SubscribedListBO) DataAccessFactory.dataHolder().get("subscribedListBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public SubscribedListHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_YELLOW + "subscribedlist handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = subscribedListBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("subscribedlist handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
