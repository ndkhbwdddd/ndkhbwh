...
public class MoreDataHandler {
    private static Logger logger = Logger.getLogger(MoreDataHandler.class);
    private MoreDataBO moreDataBO = (MoreDataBO) DataAccessFactory.dataHolder().get("moreDataBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public MoreDataHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_GREEN + "MoreDataHandler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = moreDataBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("MoreDataHandler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
