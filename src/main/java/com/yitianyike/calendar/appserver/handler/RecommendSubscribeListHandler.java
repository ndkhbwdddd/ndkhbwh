 ...
 *
 */
public class RecommendSubscribeListHandler {
    private static Logger logger = Logger.getLogger(RecommendSubscribeListHandler.class);
    private RecommendSubscribeListBO recommendSubscribeListBO = (RecommendSubscribeListBO) DataAccessFactory.dataHolder().get("recommendSubscribeListBO");

    private Map<String, String> parmMap;
    private ChannelHandlerContext ctx;
    private String content;
    
    public RecommendSubscribeListHandler(ChannelHandlerContext ctx, Map<String, String> map, String content) {
    	this.parmMap = map;
    	this.ctx = ctx;
    ...
    void process() {
    	...
    	logger.info(EnumConstants.COLOR_YELLOW + "recommendsubscribelist handler param(" + requestIndex + ") : " + parmMap + EnumConstants.COLOR_NONE);
        
    	AppResponse resp = recommendSubscribeListBO.process(parmMap, content, requestIndex);
...
//			}
		...
		logger.info("recommendsubscribelist handler resp(" + requestIndex + ") : length = (" + enResponse.length() + ") " + displayStr);
