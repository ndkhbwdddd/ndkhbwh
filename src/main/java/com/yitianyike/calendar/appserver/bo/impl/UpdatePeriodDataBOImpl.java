 ...
 *
 */
@Component("updatePeriodDataBO")
public class UpdatePeriodDataBOImpl implements UpdatePeriodDataBO {

	private static Logger logger = Logger.getLogger(UpdatePeriodDataBOImpl.class.getName());
	...
	public AppResponse process(Map<String, String> map, String content, long requestIndex) {
		...
		do {
			...
			if(minTime != null){
				long min = Long.valueOf(minTime).longValue();
				minTime = sdf.format(min); 
			}
			
			long max = Long.valueOf(maxTime).longValue();
			maxTime = sdf.format(max);  
