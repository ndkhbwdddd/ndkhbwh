...
public class RecommendSubscribeListBOImpl implements RecommendSubscribeListBO {
	...
	private static Logger logger = Logger.getLogger(RecommendSubscribeListBOImpl.class.getName());
	...
	public AppResponse process(Map<String, String> map, String content, long requestIndex) {
		...
		do {
			...
			if (uid != null) {
				...
				// break;
				// }
				String subscribeKey = uidMap.get("channel") + "-" + uidMap.get("version") + "-" + "recommend";
				...
				if (allSubscribeString == null) {
					if (redisDAO.keyExist(subscribeKey) == 0) {// redis中对应的key不存在
						...
						if (dataInfos == null || dataInfos.size() == 0) {
							...
							logger.info(
									requestIndex + " : recommend subscribe list no exist in data cache, return 500");
						} else {
						...
						appResponse.setCode(EnumConstants.CALENDAR_ERROR_500);
						logger.info(
								requestIndex + " : recommend subscribe list redis key exist, but no data, return 500");
					}
				} else {
